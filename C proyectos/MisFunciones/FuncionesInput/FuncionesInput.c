...
/** \brief Solicita un texto al usuario , valida que sea de formato CUIT , ##-12345678-X , maximo 14 caracteres.
 * elimina saltos de linea
 * \param var_dondeAsignar es donde se guarda el valor
 * \param mensaje Es el mensaje a ser mostrado
 * \return void
 */
void f_i_pedirCuit (char var_dondeAsignar[],char mensaje[])
{

    char aux[14];
    int longitudMaxString = 14;
    int i, len;
    int invalido = 0;
    ...
    do
    {
        ...
        if (invalido == 1 )
        {
            printf("\n*** Ingrese un cuit en este formato ##-12345678-X ***");
        }
        ...
        //los 2 primeros digitos y el ultimo deben ser numeros

        if( (aux[0] < '0' || aux[0] > '9') || (aux[1] < '0' || aux[1] > '9') || (aux[12] < '0' || aux[12] > '9') )
        {
            invalido = 1;
        };

        //valido que los guiones esten en su lugar
        ...
        //los 8 digitos entre los guiones deben ser numeros
        for (i=3; i<11; i++)
        {
            if (aux[i] < '0' || aux[i] > '9')
            {
 ...
 * \return void
 */
void f_i_PedirFecha (int *dia, int* mes, int *anyo,char diaEnQue[],char mesEnQue[],char anyoEnQue[])
{
